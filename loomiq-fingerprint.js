...
(function(){
  ...
  function measureShape(img){
    ...
    /* Маска форми — щоб можна було питати не «чи в межах малюнка точка», а
       «чи є під нею сам малюнок». Прямокутник меж для цього замалий: у
       кільця, у літери «О», у будь-якого лого з просвітом клік у дірку
       мусить провалюватись до шару під ним, а не брати це лого.

       Клітинка вважається зайнятою, якщо в її блоці є ХОЧ ОДИН непрозорий
       піксель. Це навмисне огрублення в бік «є»: тонку лінію в один піксель
       інакше неможливо було б ухопити пальцем. */
    var MW = 80;
    var mw = Math.max(1, Math.min(MW, cw)), mh = Math.max(1, Math.round(mw * ch / cw));
    var mask = new Uint8Array(mw * mh);
    for(var my = 0; my < mh; my++){
      var y1b = Math.min(ch, Math.round((my + 1) * ch / mh));
      for(var mx = 0; mx < mw; mx++){
        var x1b = Math.min(cw, Math.round((mx + 1) * cw / mw));
        var hit = 0;
        for(var py = Math.round(my * ch / mh); py < y1b && !hit; py++)
          for(var px = Math.round(mx * cw / mw); px < x1b; px++)
            if(d0[(py * cw + px) * 4 + 3] > THR){ hit = 1; break; }
        mask[my * mw + mx] = hit;
      }
    }
    ...
    return {
      ...
      // Маска в координатах УСЬОГО полотна, а не обрізаного шматка: той, хто
      // питає, має точку в частках від картинки цілком.
  ...
  //
  // Заміряно на ЖИВОМУ замовленні: той самий файл — 0%, копії одного логотипа
  // з двох файлів — 4.0%, різні логотипи — 4.8%. Тобто схожість пікселів
  // розділити їх не може, і поріг це не рятує: копії зводить АДРЕСА ФАЙЛУ.
  // Сам поріг живе в рушії цін (loomiq-pricing.js), бо рішення про гроші
  // ухвалює він.
  var FP_N = 12;
