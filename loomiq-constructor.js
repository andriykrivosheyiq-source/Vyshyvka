/* ══════════════════════════════════════════════════════════════════════
  ...
  (function(){
    ...
    var NAME_NO_SUFFIX = {teeover:1, tee:1, hoodieover:1, hoodieoverfleece:1, hoodie:1, hoodiezip:1, sweat:1, cap:1, tote:1};
    ...
    // Чи виходить конкретне лого за межі зони — по НЕПРОЗОРИХ пікселях (не по квадратній рамці з фоном).
    ...
    // тож із фоном (весь прямокутник) і без фону (лише контур) ціна має відрізнятись.
   ...
   вчетверо більше за найдовший напис, який колись просили. */
    ...
    function addLogo(origUrl, cleanUrl, textSpec){
      ...
      measureLayerShape(layer, url).then(function(){
        ...
        /* Панель теж треба перемалювати: до заміру шар ще не знає ні свого
           ...
           завантаження. Через це попередження про непрозорий фон не
           з'являлось узагалі — воно рахується саме із заповнення. */
        renderTabPanel();
        updatePriceBar();
              ...
              html += '<div class="pm-photo-col">' +
                '<div class="pm-photo-thumb'+(thumbDark?' on-dark':'')+'" data-id="'+l.id+'" data-side="'+gr.side+'" ' +
                  ...
                  /* Заповнення під 100% означає, що фон лишився непрозорим:
                     ...
                     Дивимось на саме заповнення, а не на те, чи вмикали
                     прибирання: найгірший випадок — коли його вже ввімкнули,
                     а воно не спрацювало. Тоді кнопка «на місці», галочка
                     стоїть, і все виглядає зробленим, хоча платять за
                     суцільний прямокутник. */
                  '<button class="pm-bg-dot'+(l.removeBg?' on':'')+
                    ((l.fill != null && l.fill >= 0.97) ? ' warn' : '')+
                    '" data-bgtoggle="'+l.id+'" title="'+
                    ((l.fill != null && l.fill >= 0.97)
                      ? (l.removeBg
                          ? 'Фон не прозорий — прибирання не впоралось. Вишивається весь прямокутник: потрібне лого з прозорим фоном'
                          : 'Фон не прозорий — вишивається весь прямокутник. Прибрати фон')
                      : (l.removeBg ? 'Фон прибрано — повернути' : 'Прибрати фон'))+
                    '" aria-label="Прибрати фон">' +
                    /* Галочка на помаранчевому читалась би як «зроблено»,
                       хоча фон так і лишився непрозорим. Тому в цьому стані
                       знак свій — оклик. */
                    ((l.fill != null && l.fill >= 0.97)
                      ? '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M12 6v8"/><path d="M12 18h.01"/></svg>'
                      : (l.removeBg
      ...
      orderedLayers().forEach(function(x){
        ...
        if(m.mode === 'grid'){
          ...
          if(!isTxt && (l.fill == null || l.fill >= 0.97))
            note += l.removeBg
              /* Прибирання фону вже вмикали, а заповнення все одно 100% —
                 отже воно не спрацювало. Радити натиснути ту саму кнопку
                 було б знущанням: тут потрібен інший файл. */
              ? ' · ⚠ фон не прозорий — автоматичне прибирання не впоралось, ' +
                'попросіть лого з прозорим фоном (PNG або вектор)'
              : ' · ⚠ фон не прозорий — рахується весь прямокутник, натисніть «Прибрати фон»';
        }
    ...
    // Легке оновлення лише ціни/розміру під час жесту (без повного renderTiers/scrollIntoView — щоб не смикало інтерфейс)
    ...
    var detailTab = 'about';
    var SPECS = [
      ['Матеріал','100% бавовна'],['Щільність','240 г/м²'],['Крій','оверсайз'],
      ['Формат','унісекс'],['Нанесення','DTF друк та вишивка']
    ];
    var PRINT_COMPARE = [
      {name:'DTF-друк', bg:'#f0f4ff', color:'#2d4ef5', price:'від 540 грн',
    ...
    var PM_REVIEWS = [
      {name:'Олексій К.', date:'2 дні тому', stars:5, text:'Дуже якісна футболка! Принт чіткий, колір не блякне.'},
      {name:'Марина В.', date:'тиждень тому', stars:5, text:'Дуже швидко зробили і доставили. Замовлятиму ще.'},
      {name:'Дмитро С.', date:'2 тижні тому', stars:5, text:'Оверсайз крій — саме те. Тканина приємна, не тягнеться.'},
      {name:'Ольга С.', date:'3 тижні тому', stars:5, text:'Замовляли корпоративні футболки. Всі задоволені, будемо знову.'},
