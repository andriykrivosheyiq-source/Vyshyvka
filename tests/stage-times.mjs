...
console.log('');
console.log('═══ СКІЛЬКИ ЦЕ ВЖЕ СТОЇТЬ — ВИДНО В КАРТЦІ ═══');
/* Питання «чому замовлення не рухається» доти вимагало прочитати історію до
   кінця. Відповідь на нього — одне число: скільки трек стоїть у поточному
   стані. Рахувати нема чого заводити — кожен рух треку вже пишеться з часом. */
const age = await p.evaluate(() => {
  const o = orders[0];
  const год = h => new Date(Date.now() - h * 3600e3).toISOString();
  o.tracks = Object.assign({}, o.tracks, { prod:'work' });
  o.trackHist = [{ t:'prod', s:'work', at: год(30) }];
  const середнє = trackAgeHtml(o, 'prod', 'work');
  o.trackHist = [{ t:'prod', s:'work', at: год(100) }];
  const давнє = trackAgeHtml(o, 'prod', 'work');
  o.trackHist = [{ t:'prod', s:'work', at: год(1) }];
  const свіже = trackAgeHtml(o, 'prod', 'work');
  const початок = trackAgeHtml(o, 'prod', (trackSteps('prod')[0] || {}).key);
  return { середнє, давнє, свіже, початок };
});
console.log('   30 год: ' + (age.середнє.match(/>([^<]+)</) || [])[1] +
            ' · 100 год: ' + (age.давнє.match(/>([^<]+)</) || [])[1] +
            ' · 1 год: «' + age.свіже + '»');
ok(/30 год/.test(age.середнє),
  'біля треку стоїть, скільки він у цьому стані',
  'віку треку не видно: ' + age.середнє);
ok(/is-hot/.test(age.давнє) && /4 дн/.test(age.давнє),
  'понад три доби позначаємо окремо — це вже не «йде робота», а «забули»',
  'давній трек не виділився: ' + age.давнє);
ok(age.свіже === '' && age.початок === '',
  'перші дві години мовчимо, і трек, який ще не починали, числа не отримує — ' +
    'інакше свіже замовлення виглядало б простроченим із першої хвилини',
  'число зʼявилось зарано');
