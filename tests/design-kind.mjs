...
if(errs.length) bad++;
console.log('');
console.log('═══ «ЗБЕРЕГТИ ПОЗИЦІЮ» НЕ ЗАБИРАЄ ВИД НАЗАД ═══');
/* Андрій: «я змінюю на надпис, потім зберігаю і воно наче збереглось,
   перезаходжу, а воно не збереглось».

   Вид дизайну — рішення менеджера, а не властивість файлу: воно міняє
   ставку за площу, а отже й ціну. Конструктор про таке рішення не знає
   нічого — він бачить шар і щоразу присилає «картинка». Тому кожне
   «Зберегти позицію» тихо повертало вид назад.

   Переносимо за ВІДБИТКОМ малюнка, а не за номером у списку: номер
   зсувається, щойно додали чи прибрали нанесення. */
{
  const adm = fs.readFileSync(path.join(ROOT, 'loomiqadmin.html'), 'utf8');
  const блок = adm.slice(adm.indexOf('const wasD = was.desc, nuD = nu.desc;'),
                         adm.indexOf('const wasD = was.desc, nuD = nu.desc;') + 1400);
  ok(/byFp\[String\(fp\)\] = k/.test(блок) && /wasD\.designs\.forEach/.test(блок),
    'вид дизайну переживає збереження позиції з конструктора',
    'конструктор щоразу повертає «картинка» — рішення менеджера губиться');
  ok(/byFp\[String\(fp \|\| ''\)\]/.test(блок),
    'переносимо за відбитком малюнка, а не за номером у списку',
    'вид привʼязаний до номера — додали нанесення, і він переїхав на чужий дизайн');
  ok(/setItemKindFix\(null, fp, byFp\[fp\], target\)/.test(блок),
    'рішення лягає і в мапу замовлення — воно про малюнок, а не про цю позицію',
    'рішення лишається тільки в позиції: приберуть її — і сусідня порахується інакше');
  ok(!/kinds\[di\] = 'img'/.test(блок),
    'замінили файл — вид повертається до автоматичного, і це навмисно',
    'чужий вид переноситься на новий малюнок');
}
