...
/* Номер групи тут не важить: повну підготовку несе найпоширеніший дизайн,
   тож ескізом стає той, що лишився, — байдуже, під яким він номером. */
ok(/ескіз №\d/.test(good.out) && /рівно один раз/.test(good.out),
