...
/* Треків вісім: пʼять, у яких є своя дошка й своя людина, і три без
   дошки — тест, контроль, відправка. Останні нікуди не поділись: їхні
   кроки проставляють у картці, і на них тримаються ворота тиражу, брак і
   ТТН. Просто окремої дошки під них немає, бо немає й окремої ролі. */
ok(paid.has && paid.sels.length === 8,
  'в оплаченому замовленні всі треки, кожен зі своїм вибором',
  'треків не вісім: ' + paid.sels.join(','));
ok(paid.labels.join(',') ===
     'Аккаунт-менеджер,Графічний дизайнер,Вишивальний дизайнер,Виробництво,Закупки,Тест,Контроль,Відправка',
  'треки названі так, як про них говорять уголос — ролями, а не абстракціями',
...
ok(moved.chips.length === 1 && /Графічний дизайнер · На перевірці/.test(moved.chips[0]),
  'на дошці видно рівно те, що зрушило: «Графічний дизайнер · На перевірці»',
...
ok(/Графічний дизайнер ✓/.test(done.chip || '') && done.tick,
...
ok(feed.some(x => /Графічний дизайнер: Макет погоджено/.test(x)) &&
   feed.some(x => /Графічний дизайнер: На перевірці/.test(x)),
...
/* Редактор кроків показує ВСІ воронки, включно з тими, що не мають своєї
   дошки: кроки тесту, контролю й відправки однаково проставляють у картці,
   і налаштовувати їх десь треба. */
ok(tabs.length === 9 && tabs[0] === 'Продажі',
  'редактор один на всі воронки: продаж і вісім треків',
  'вкладок не девʼять: ' + tabs.join(','));
