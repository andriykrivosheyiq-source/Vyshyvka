...
const read = fr => fr.evaluate(()=>{
  ...
  return {
    ...
    позиції: [...document.querySelectorAll('.est-i')].map(c=>({
      ...
      кількість: ((c.querySelector('.est-q-v')||{}).textContent||'').trim(),
      лічильник: c.querySelectorAll('.est-q-b').length,
...
/* Темна смуга: кошторис — момент, коли сторінка змінює тон і людина розуміє,
   ...
   Стережемо намір, а не конкретний піксель: фон має бути темним, але не
   чистим чорним. Точне значення — справа дизайн-системи, і прибивати його
   цвяхом у тесті означає, що кожне уточнення відтінку падає як помилка. */
const rgb = (String(d.фон).match(/\d+/g) || []).map(Number);
const lum = rgb.length >= 3 ? (rgb[0] + rgb[1] + rgb[2]) / 3 : 255;
ok(lum < 60 && lum > 8,
  'кошторис на темній смузі, і це графіт, а не чорна діра: ' + d.фон,
  'фон: ' + d.фон);
/* «Вартість замовлення», а не «Кошторис»: назва розділу має казати, що в
   ньому, тими словами, якими про це думає клієнт. */
ok(d.шапка === 'Вартість замовлення' && d.підпис === '1 позиція · 4 одиниці',
  'шапка: «' + d.шапка + ' · ' + d.підпис + '» — спершу склад, потім обсяг',
  'шапка: ' + d.шапка + ' / ' + d.підпис);
/* Кількість у кошторисі не просто написана — її тут же й міняють: замовлення
   людина зводить у цьому блоці, і бігати назад до картки заради однієї штуки
   немає сенсу. */
ok(d.позиції.length === 1 && /^−\s*4\s*\+\s*1 450 грн\/шт$/.test(d.позиції[0].множення),
  'позиція одним рядком: лічильник 4 і 1 450 грн/шт',
  'множення: ' + JSON.stringify(d.позиції.map(x=>x.множення)));
ok(d.позиції.every(x => x.лічильник === 2),
  'кількість можна змінити просто в кошторисі',
  'лічильника немає: ' + JSON.stringify(d.позиції.map(x=>x.лічильник)));
...
/* Одна кнопка, і без стрілки. «Завантажити PDF» стояла поруч і важила
   стільки ж, скільки дія, заради якої написана вся сторінка; переслати
   кошторис тепер можна кнопкою «Поділитися» в шапці. */
/* Головна дія одна — «Підтвердити». «Поділитися» під нею не конкурує:
   інша дія, інший рядок, прозора кнопка. Двох кнопок з тією самою дією
   («Підтвердити» + «Оформити») тут бути не може — це читалось би як вибір
   там, де вибору немає. */
ok(d.кнопки.length === 2 && d.кнопки.every(x=>x.видно) &&
   d.кнопки[0].текст === 'Підтвердити пропозицію' &&
   d.кнопки[1].текст === 'Поділитися',
  'під сумою дія і пересилання: ' + JSON.stringify(d.кнопки.map(x=>x.текст)),
...
ok(d.розмірШапки >= 24,
  'заголовок розділу видно (' + d.розмірШапки + 'px), а не дрібним підписом',
