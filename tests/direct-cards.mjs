...
/* Людина на фото стоїть під кутом, а логотип лягає пласким прямокутником —
   і картка виглядає наліпкою. Полотно перспективи не вміє: drawImage знає
   лише зсув, масштаб і поворот. Тому чотирикутник рахується перетворенням,
   а малюється дрібною сіткою, де кожна клітинка вже пласка. */
const warp = await fr.evaluate(() => {
  const L = window.LQCards;
  const H = L.homography([[0,0],[100,0],[100,100],[0,100]]);
  const прямо = L.hmap(H, 0.5, 0.5);
  /* Нахилений чотирикутник: верх вужчий за низ — саме так виглядає груди
     людини, поверненої боком. Середина при проективному перетворенні
     зміщується до вузького краю; у простого «розтягування» вона лишилась
     би рівно посередині. */
  const S = L.homography([[20,0],[80,0],[100,100],[0,100]]);
  const косо = L.hmap(S, 0.5, 0.5);
  const верх = L.hmap(S, 0.5, 0);
  const низ = L.hmap(S, 0.5, 1);
  return { прямо, косо, верх, низ };
});
console.log('   прямий: ' + warp.прямо.map(Math.round).join(',') +
            ' · нахилений центр: ' + warp.косо.map(v => Math.round(v)).join(','));
ok(Math.abs(warp.прямо[0] - 50) < 0.01 && Math.abs(warp.прямо[1] - 50) < 0.01,
  'на прямому чотирикутнику центр лишається центром',
  'перетворення бреше вже на прямому: ' + warp.прямо.join(','));
ok(Math.abs(warp.верх[0] - 50) < 0.01 && Math.abs(warp.низ[0] - 50) < 0.01,
  'вісь симетрії лишається віссю — нанесення не з\'їжджає вбік',
  'вісь поїхала: ' + warp.верх.join(',') + ' / ' + warp.низ.join(','));
/* Верх чотирикутника вужчий — отже, він «далі». Середина зображення при
   проективному перетворенні йде саме туди, до дальнього краю: у простого
   розтягування вона лишилась би рівно на 50. */
ok(warp.косо[1] < 49.5,
  'перетворення саме проективне: середина зміщується до вузького, дальнього ' +
    'краю — у простого розтягування вона лишилась би посередині',
  'перспективи немає, це звичайне розтягування: ' + warp.косо.join(','));
...
/* Виріб має бути цілий: кадри приходять різних пропорцій, і «заповнити
   плитку» на високому мокапі означало б зрізати худі рукави й низ. Решту
   плитки добирає її колір — той самий, що фон мокапа. */
ok(/Math\.min\(cov, fit\)/.test(src),
  'перекриття плитки беремо лише доти, доки воно не ріже сам виріб',
