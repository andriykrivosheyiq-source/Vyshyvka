...
/* Підказка називає два числа: скільки додати і скільки за це буде знижки на
   ВСЕ замовлення. Знижка — різниця між тим, скільки той самий більший тираж
   коштував би за нинішньою ціною за штуку, і тим, скільки він коштує за
   новою. Тест набирає названу кількість і рахує цю різницю сам. */
const m = /Додайте ще (\d+)\D+знижку ([\d\s ]+) грн/.exec(start.обіцянка);
ok(!!m, 'під сумою є підказка про найближчий поріг: «' + start.обіцянка + '»',
  'порогу не запропоновано зовсім: «' + start.обіцянка + '»');

if(m){
  const need = +m[1], extra = num(m[2]), was = num(start.разом);
  const unitWas = Math.round(was / start.кількість);
  await bump(need);
  ...
  const asIs = (start.кількість + need) * unitWas;
  ok(Math.abs(asIs - num(after.разом) - extra) <= 2,
    'знижка на все замовлення збіглася: ' + extra + ' грн',
    'обіцяли знижку ' + extra + ', а виходить ' + (asIs - num(after.разом)));
  /* Тут поріг настільки близький, що виробів більше, а платити менше. */
  ok(num(after.разом) < was,
    'замовлення подешевшало, хоч виробів більше: ' + was + ' → ' + num(after.разом),
    'сума зросла: ' + was + ' → ' + num(after.разом));
}
...
const m2 = /Додайте ще (\d+)\D+знижку ([\d\s ]+) грн/.exec(s2.обіцянка);
ok(!!m2,
  'на маленькому замовленні підказка теж є: «' + s2.обіцянка + '»',
  'під сумою порожньо — підказки немає там, де вона найпотрібніша: «' +
    s2.обіцянка + '»');
if(m2){
  const need = +m2[1], extra = num(m2[2]);
  const wasU = num((/([\d\s ]+) грн\/шт/.exec(
