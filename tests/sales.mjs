/* Продажна воронка не дає загубити угоду.
   ...
   Тому «клієнт мовчить» живе не на дошці, а на КАРТЦІ, трьома способами:
     ...
     — «відкласти до дати» — для домовленості «повернемось у листопаді».
       Картка йде з дошки й повертається сама, на ТОЙ САМИЙ етап: етап не
       змінився, змінилась лише наша увага.
...
await p.click('[data-snooze="7"]');
await p.waitForTimeout(1300);
const n1 = await onBoard();
const d1 = await deal();
ok(n1 === n0 - 1 && /Відкладено до/.test(d1.t),
  'відклали на тиждень — картка пішла з дошки',
