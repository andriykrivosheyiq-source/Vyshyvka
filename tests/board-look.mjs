...
console.log('');
console.log('═══ КНОПКА «КП» — ПРЯМО НА КАРТЦІ ═══');
/* До редактора пропозиції вела одна дорога — відкрити картку й знайти
   кнопку всередині; а складають його щодня й до кожного замовлення.

   Перевіряємо на ЖИВІЙ дошці, а не в розмітці: малювальників картки в
   адмінці кілька, і кнопку вже одного разу додали не в той — на дошці її
   від того не зʼявилось. */
const кп = await p.evaluate(() => {
  const t = document.querySelector('#board .ticket');
  if(!t) return { картки:false };
  const b = t.querySelector('[data-act="kp"]');
  if(!b) return { картки:true, кнопка:false };
  const r = b.getBoundingClientRect();
  return { картки:true, кнопка:true, текст:(b.textContent || '').trim(),
           видно: r.width > 0 && r.height > 0,
           влазить: b.scrollWidth <= b.clientWidth + 1 };
});
if(!кп.картки){
  /* Карток на дошці немає — перевіряємо хоча б те, що кнопка стоїть у
     ТОМУ малювальнику, який дошку й малює. Один раз її вже додали не
     туди: у файлі вона була, на дошці — ні. */
  const src2 = fs.readFileSync(path.join(ROOT, 'loomiqadmin.html'), 'utf8');
  const i0 = src2.indexOf('function ticketSummaryHtml');
  const i1 = src2.indexOf('function floorTicketHtml');
  console.log('  (карток на дошці немає — перевіряємо малювальник)');
  ok(i0 > 0 && i1 > i0 && /kpBtnHtml\(o\)/.test(src2.slice(i0, i1)),
    'кнопка «КП» стоїть у малювальнику карток дошки',
    'кнопки немає в ticketSummaryHtml — вона знову не там');
} else {
  console.log('  кнопка: ' + (кп.кнопка ? '«' + кп.текст + '»' : 'немає'));
  ok(кп.кнопка && кп.видно,
    'кнопка «КП» стоїть на самій картці Канбану, а не лише в розмітці',
    'кнопки на картці немає');
  ok(!кп.кнопка || кп.влазить,
    'підпис влазить у кнопку цілком',
    'підпис обрізало: «' + кп.текст + '»');
}

console.log('');
console.log('═══ ТРИ СТУПЕНІ КНОПКИ «КП» ═══');
/* Менеджер веде десятки замовлень, і питання до кожного одне: ми вже щось
   надіслали і чи відповіли. Доти позначка була одна — «пропозиція є», — і
   «зібрав, але нікому не показав» виглядало так само, як «надіслав, клієнт
   мовчить». Читається як у месенджері, тож пояснювати нічого не треба. */
{
  const adm = fs.readFileSync(path.join(ROOT, 'loomiqadmin.html'), 'utf8');
  const ст = await p.evaluate(() => {
    const f = window.kpBtnHtml;
    if(typeof f !== 'function') return null;
    const текст = o => (f(o).match(/>([^<]*)<\/button>/) || [,''])[1].trim();
    return {
      нема:  текст({}),
      є:     текст({ offerToken:'t' }),
      відповів: текст({ offerToken:'t', cardsAt:'2026-01-01', cardsReplyAt:'2026-01-02' })
    };
  });
  if(!ст){
    console.log('  (кнопка не винесена в window — перевіряємо джерело)');
    ok(/КП\$\{s === 2 \? ' ✓✓'/.test(adm),
      'у кнопці три ступені: без позначки, одна галочка, дві',
      'ступенів не три');
  } else {
    console.log('   ' + JSON.stringify(ст));
    ok(ст.нема === 'КП',
      'КП ще немає — кнопка без позначок',
      'без пропозиції кнопка показує «' + ст.нема + '»');
    ok(ст.є === 'КП ✓',
      'пропозиція зібрана — одна галочка',
      'зібрана пропозиція показує «' + ст.є + '»');
    ok(ст.відповів === 'КП ✓✓',
      'клієнт озвався після карток — дві галочки',
      'відповідь клієнта показує «' + ст.відповів + '»');
  }
  /* Чорна, бо на картці це головна дія: стрілки поруч лише пересувають по
     етапах, а пропозиція — те, заради чого замовлення існує.

     Міряємо ОБЧИСЛЕНИЙ колір на живій кнопці, а не шукаємо рядок у файлі.
     Правило в адмінці вже стояло — і не працювало: нижче в тому ж файлі
     лежить `.tc-act{background:transparent}` тієї самої ваги, а перемагає
     останнє. У джерелі все виглядало правильно, на екрані кнопка лишалась
     світлою, і Андрій двічі сказав, що чорної не бачить. */
  const колір = await p.evaluate(() => {
    const host = document.createElement('div');
    host.className = 'ticket';
    host.innerHTML = '<div class="tc-acts">' + window.kpBtnHtml({ offerToken:'t' }) + '</div>';
    (document.getElementById('board') || document.body).appendChild(host);
    const b = host.querySelector('[data-act="kp"]');
    const s = getComputedStyle(b);
    const out = { фон:s.backgroundColor, текст:s.color, тінь:s.opacity };
    host.remove();
    return out;
  });
  console.log('   ' + JSON.stringify(колір));
  const темний = c => {
    const m = String(c).match(/(\d+),\s*(\d+),\s*(\d+)/);
    return m && (+m[1] + +m[2] + +m[3]) / 3 < 70;
  };
  ok(темний(колір.фон),
    'кнопка справді чорна на екрані, а не лише в правилі',
    'фон кнопки лишився світлим: ' + колір.фон);
  ok(!темний(колір.текст),
    'підпис на ній світлий',
    'підпис не читається на чорному: ' + колір.текст);
  ok(+колір.тінь === 1,
    'і не блідне разом з рештою дій — її видно, не наводячи мишу',
    'кнопка прозора, доки не наведеш: ' + колір.тінь);
}
