...
/* ── РЕКОМЕНДОВАНА СТОЇТЬ НА СВОЄМУ МІСЦІ У СКЛАДІ ─────────────────────
   Перша група дизайнів входить у підготовку макета, кожна наступна коштує
   ескіз — а яка група буде першою, вирішує порядок, у якому дизайни
   приходять у рушій. Адмінка дописувала рекомендовану в кінець списку,
   конструктор тримає її на власному місці. Ті самі позиції рахувались двома
   різними списками — звідси «у підрахунку одна ціна, а зліва інша». */
console.log('');
console.log('═══ ПОРЯДОК СКЛАДУ ОДНАКОВИЙ В АДМІНЦІ Й У КОНСТРУКТОРІ ═══');
const порядок = await p.evaluate(() => {
  const A = Array.from({ length:144 }, (_, i) => (i * 7) % 10).join('');
  const B = Array.from({ length:144 }, (_, i) => (i * 3 + 1) % 10).join('');
  const шар = (fp, url) => ({ id:'L' + fp.slice(0, 3), url, fp, scale:1, frac:.3,
                              fx:.3, fy:.3, ar:1, fill:.8, w:60, h:60 });
  const поз = (kind, name, gid, qty, fp, url, base) => ({
    kind, name, garmentId:gid, qty, unitPrice:0, price:0,
    config:{ logos:{ front:[шар(fp, url)] } },
    desc:{ method:'embro', gid, units:qty, base, coefPart:500, basePart:150,
           minPart:0, pieceFee:75, bare:false, designs:[fp], designKinds:['img'],
           designMm2:[4000], dtfCols:[] } });
  const o = orders[0];
  /* РЕКОМЕНДОВАНА СТОЇТЬ ПЕРШОЮ в списку позицій — саме тоді порядок і
     важить. Її дизайн інший, ніж на основній. */
  o.items = [ поз('reco', 'Футболка', 'tee', 8, B, 'https://cdn.test/b.png', 440),
              поз('main', 'Кепка', 'cap', 8, A, 'https://cdn.test/a.png', 360) ];
  repriceOrder(o);
  const рек = o.items[0];
...
console.log('  адмінка ' + порядок.адмінка + ' ₴ · конструктор ' + порядок.конструктор +
            ' ₴ · ' + порядок.рядки.join(' · '));
ok(порядок.адмінка === порядок.конструктор,
  'ціна рекомендованої однакова — обидві рахують той самий склад у тому самому порядку',
