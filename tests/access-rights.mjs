/* Роздавання доступів: видалення, зрозумілі назви, запрошення поштою.
   ...
   Перевіряємо:
     ...
     — у виборі людини рівно ті ролі, що є у вкладці «Ролі», по одному рядку
       на роль: напрям задається розділами самої ролі, а не окремим полем;
...
console.log('');
console.log('═══ У ВИБОРІ ЛЮДИНИ — РІВНО ТІ РОЛІ, ЩО Є ═══');
/* Якийсь час напрям (B2B/B2C) зберігався ОКРЕМИМ полем людини, а щоб його
   десь вибрати, кожна роль домножувалась у списку на три рядки: «Менеджер
   B2B», «Менеджер B2C», «Менеджер · B2B і B2C». Виходило те, від чого й
   тікали: у вкладці «Ролі» вісім записів, а у виборі людини двадцять пʼять,
   і половини з них не існує.

   А напрям при цьому не робив нічого, крім вмикання чотирьох розділів меню
   — Канбану, двох чатів і B2C, — і ті чотири розділи вже й так є галочками
   в самій ролі. Другий вимикач на ті самі двері. */
const ролі = await p.evaluate(() => {
  ...
  return { вибір:opts, ролі:roleDefs().map(r => r.name),
           значення:[...(sel ? sel.options : [])].map(o => o.value) };
});
console.log('   ' + JSON.stringify(ролі.вибір));
ok(ролі.вибір.join() === ролі.ролі.join(),
  'у виборі людини рівно ті ролі, що у вкладці «Ролі», — одним рядком на роль',
  'вибір розійшовся з ролями: ' + JSON.stringify(ролі));
ok(!ролі.значення.some(v => v.indexOf('|') >= 0),
  'жодних вигаданих «Менеджер B2B» — роль передається ключем, і більше нічим',
  'у значенні досі щось, крім ролі: ' + JSON.stringify(ролі.значення));
ok(!/memberDirs|canDir\(/.test(fs.readFileSync(path.join(ROOT, 'loomiqadmin.html'), 'utf8')),
  'напряму як окремого поля людини більше немає — його задають розділи ролі',
  'напрям досі живе поруч із роллю');

/* Напрям тепер задається розділами: Канбан і «Чати B2B» — це B2B, «B2C» і
   «Чати B2C» — це B2C. Менеджер одного напряму так само не знає, що існує
   інший, — просто це сказано галочкою, а не окремим полем. */
const розділи = await p.evaluate(() => {
  const було = JSON.stringify(contentData.team || []);
  const булиРолі = contentData.roles;
  const тест = nav => {
    contentData.roles = [{ key:'проба', name:'Проба', ui:'manager', nav:nav,
      boards:['sale'], see:['client'], can:['edit'],
      navAll:ZONE_NAV.map(z => z.key), canAll:ZONE_CAN.map(z => z.key) }];
    contentData.team = [{ email:myEmail(), name:'Я', role:'проба' }];
    teamDraft = null; applyRoleUi();
    return [...document.querySelectorAll('.nav button[data-view]')]
      .filter(b => !b.hidden).map(b => b.dataset.view);
  };
  const b2b = тест(['today','board','chats']);
  const b2c = тест(['today','design','chatsc']);
  contentData.roles = булиРолі;
  contentData.team = JSON.parse(було); teamDraft = null; applyRoleUi();
  return { b2b, b2c };
});
console.log('   ' + JSON.stringify(розділи));
/* Розділів, яких не існувало, коли роль зберігали, у її списку немає — і без
   добору за старим їх не побачив би ніхто з уже заведених. Саме так одного
   разу й зникли чати. */
const добір = await p.evaluate(() => {
  const було = JSON.stringify(contentData.roles || []);
  /* Рівно так роль і виглядала до появи чатів: ні `chats`, ні `navAll`. */
  contentData.roles = [{ key:'стара', name:'Стара', ui:'manager',
    nav:['today','board'], boards:['design'], see:['client'], can:['edit'] }];
  const nav = presetOf('стара').nav.slice();
  contentData.roles = JSON.parse(було); teamDraft = null;
  return nav;
});
console.log('   ' + JSON.stringify(добір));
ok(добір.indexOf('chats') >= 0 && добір.indexOf('design') >= 0 &&
   добір.indexOf('chatsc') < 0,
  'нові розділи доходять і до ролей, збережених раніше: хто веде Канбан — бачить і його чати',
  'новий розділ не побачить ніхто з уже заведених: ' + JSON.stringify(добір));
ok(розділи.b2b.indexOf('board') >= 0 && розділи.b2b.indexOf('design') < 0,
  'роль B2B бачить Канбан і не бачить B2C — вона навіть не знає, що той є',
  'ролі B2B видно чужий напрям: ' + JSON.stringify(розділи));
ok(розділи.b2c.indexOf('design') >= 0 && розділи.b2c.indexOf('board') < 0,
  'роль B2C бачить свій напрям і не бачить Канбану',
  'ролі B2C видно Канбан: ' + JSON.stringify(розділи));
