...
/* Еталон переписаний удруге, і теж свідомо. Дві причини, обидві — виправлення.

   ПЕРША. Група варіантів дає в замовлення ОДНУ позицію, і її тираж —
   частина складу, а не здогад. Доти базова рахувалась сама по собі (тираж
   5), хоч поруч у пропозиції стояла група на 5 штук, яку клієнт точно
   візьме. Тепер склад — це основні позиції ПЛЮС по одному варіанту з кожної
   групи, тобто 10 виробів.

   ДРУГА. Опис позиції, що йшов у рушій, не завжди ніс ознаку товару: варіант
   і рекомендована діставали свою, а основні позиції поруч із ними — ні.
   Через це знижка на САМ ВИРІБ рахувалась по різних відрах: п'ять футболок
   в одному, п'ять «нічиїх» у другому, і поріг у 10 штук не спрацьовував
   ніколи. Тепер ознака товару стоїть скрізь, і десять футболок — це десять
   футболок.

   На базовій футболці:
     було : round(540×1.00 + 260×0.85) + 20 + 900/5  = 540+221 → 761 + 20 + 180 = 961
     стало: round(540×0.92 + 260×0.81) + 20 + 900/10 = round(634.8+210.6) → 707 + 20 + 90 = 817

   Решта — тією самою арифметикою: у складі 10 виробів, тож коефіцієнт
   нанесення 0.81 і коефіцієнт виробу 0.92 діють на всіх.

   Рекомендована рахується «склад + вона» — тобто знижку за тираж бере від
   15 виробів, бо шиються вони разом. А от за МАКЕТ вона не платить: малюнок
   той самий, що в складі, і склад уже оплатив його повністю. Брати вдруге
   за ту саму роботу нема за що, тож із її ціни йде частка разової — 60 ₴. */
const ЕТАЛОН = [
  { назва:'Футболка базова', сегмент:'склад',    ціна:817, було:1720, знижка:53 },
  { назва:'Футболка поло',   сегмент:'варіант',  ціна:955, було:1870, знижка:49 },
  { назва:'Футболка оверсайз', сегмент:'варіант',ціна:891, було:1800, знижка:51 },
  { назва:'Кепка',           сегмент:'реко',     ціна:762, було:1750, знижка:56 }
];
