...
const OFFER={orderId:'1001100',client:{name:'Андрій',company:'Стоматологія'},
  ...
  /* У першої стоїть `tierQty`: ціну їй порахували на ВЕСЬ тираж разом —
     тридцять худі плюс її десять, — бо логотип той самий і макет уже
     готовий. Саме це число має стояти в підписі. */
  reco:[Object.assign(item('Футболка базова','Чорний',10,678), { tierQty:40 }),
        item('Футболка поло','Білий',5,975), item('Кепка','Чорний',1,606)],
...
ok(kits.join(' | ') === 'від 40 штук | від 5 штук | від 1 штуки',
  'кожна плитка каже, на який тираж порахована її ціна',
  'підписи не ті: ' + JSON.stringify(kits));
/* Це і є та помилка, яку Андрій побачив у прорахунку: базовий товар
   чотири штуки, рекомендований теж чотири, ціна порахована на вісім — а
   підпис казав «від 4 штук». Тобто називав ціну восьми штук ціною
   чотирьох, і виглядало це як помилка в рахунку. */
ok(kits[0] === 'від 40 штук',
  'рекомендована показує тираж, на який справді порахована, а не свою кількість',
  'підпис називає ціну всього тиражу ціною самої позиції: ' + kits[0]);
