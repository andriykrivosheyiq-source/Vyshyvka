...
/* ── ВИДНО, ПРО ЯКИЙ МАЛЮНОК РЯДОК ─────────────────────────────────────
   «Два дизайни, хоча там один» — це не завжди помилка рахунку: підготовка
   макета належить усьому замовленню, а ескіз — конкретному малюнку, і на
   одному виробі вони стоять поруч. Перевірити це на екрані було нічим: два
   рядки «картинка», і про який саме малюнок ідеться — невідомо. */
console.log('');
console.log('═══ У РЯДКУ РАЗОВОЇ ВИДНО САМ МАЛЮНОК ═══');
const малюнки = await p.evaluate(() => {
  const box = document.getElementById('pmMgrCalc');
  if(!box) return { немає:true };
  const rows = [...box.querySelectorAll('tr')]
    .filter(t => /Підготовка макета|Додатковий ескіз/.test(t.innerText));
  return { рядків: rows.length,
           зМалюнком: rows.filter(t => t.querySelector('img')).length,
           адреси: rows.map(t => ((t.querySelector('img') || {}).src || '').slice(-12)) };
});
console.log('  ' + JSON.stringify(малюнки));
ok(малюнки.рядків > 0 && малюнки.зМалюнком === малюнки.рядків,
  'кожен рядок разової показує свій малюнок — видно, за що саме гроші',
  'рядки без малюнка: ' + JSON.stringify(малюнки));

console.log('');
console.log('═══ АДРЕСИ ФАЙЛІВ ЇДУТЬ У ОПИС ДЛЯ РУШІЯ ═══');
const джерело = fs.readFileSync(path.join(ROOT, 'loomiqadmin.html'), 'utf8');
ok(/fillDesignUrls\(o\)/.test(джерело.slice(джерело.indexOf('function calcSeedFor'),
                                            джерело.indexOf('function calcSeedFor') + 900)),
  'адмінка кладе адреси в склад, який їде в конструктор — інакше два примірники ' +
    'рушія рахують те саме замовлення по-різному',
  'адреси в кадр не їдуть');
