...
const око = await p.evaluate(async () => {
  ...
  chatClose(); chatOpen(o, 'crm');
  await new Promise(r => setTimeout(r, 400));
  /* ВІДКРИЛИ — ОТЖЕ ПРОЧИТАЛИ. Доти прочитанням вважалась тільки відповідь
     або натиск на око, і виглядало це поломкою: розмову прочитано, а число
     біля розділу так і висить. Риска «нові повідомлення» при цьому лишається
     на місці саме на цьому показі — інакше незрозуміло, звідки читати. */
  ...
  /* Назад у непрочитані — тим самим оком. */
  ...
  return { до, назад, після, післяТакту };
});
console.log('   відкрив: чекає ' + око.до.чекає + ' · зняв: ' + око.назад.непрочитаних +
            ' · знову прочитав: чекає ' + око.після.чекає +
            ' · після такту: чекає ' + око.післяТакту.чекає);
ok(око.до.кнопка && !око.до.чекає && око.до.світиться,
  'відкрив і прочитав — розмова вже не чекає, позначка світиться',
  'відкривання не рахується прочитанням: ' + JSON.stringify(око.до));
ok(око.до.риска,
  'а риска «нові» на цьому показі лишається — видно, звідки читати',
  'риска зникла тієї ж миті, і незрозуміло, з якого місця читати');
...
ok(!око.після.чекає && око.після.світиться && !око.після.риска,
  'натиснув знову — кружечок зник, кнопка засвітилась, риска прибралась',
