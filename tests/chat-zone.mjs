   ...
   прорахунок сьогодні, «до цього зазвичай беруть» — через тиждень. */
{
  ...
  if(індекс < 0) console.log('  (скрипта в наборі немає — пропускаємо живу перевірку)');
    ...
    ok(панель && !панель.поле,
      'спершу показуємо самі картки, а не мовчки набиваємо поле',
      'скрипт відразу вставив текст у поле: ' + JSON.stringify(панель && панель.поле));
    ...
    ok(!!готове.поле.trim(),
      'а текст скрипта — у поле вводу, щоб менеджер перечитав і підкоригував',
