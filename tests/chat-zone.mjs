   ...
   документації не вгадати, тому код пробує способи сам. */
{
  ...
  /* Sitniks не передає нашу адресу далі, а завантажує картинку до себе й
     аж потім віддає в Instagram. На великому кадрі це довше за дві
     секунди — і правильний спосіб оголошувався невдалим, після чого
     перебір слав клієнту ще одне повідомлення, і ще, і ще. */
  ok(/for\(const pause of \[/.test(pic),
    'стрічку перепитуємо кілька разів — повільна картинка не має виглядати невдалою',
    'на перевірку дається одна коротка пауза: повільний кадр тягне за собою весь перебір');
  ok(/st\.blind = before < 0/.test(pic),
    'коли перевірити не вдалось, це памʼятається — «надіслано» і «здається, надіслано» різні речі',
    'неперевірену відправку видають за перевірену');
