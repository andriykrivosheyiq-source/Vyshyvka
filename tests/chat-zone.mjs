   ...
   прорахунок сьогодні, «до цього зазвичай беруть» — через тиждень. */
{
  ...
  if(індекс < 0) console.log('  (скрипта в наборі немає — пропускаємо живу перевірку)');
    ...
    // Панель лише готує — тиснемо «У поле» й дивимось, що вийшло.
    const готове = await p.evaluate(async () => {
      document.querySelector('[data-cd-go]').click();
      await new Promise(r => setTimeout(r, 400));
      return { панель: !!document.querySelector('.cw-cards'),
    ...
    ok(панель && панель.позначених > 0 && /У поле/.test(панель.кнопка || ''),
    ...
    ok(готове.плиток > 0 && готове.карток === готове.плиток,
      'обрані картки лягають у смужку над полем — звідти їх ще видно й можна зняти',
      'картки не потрапили у смужку: ' + JSON.stringify(готове));
    ok(!!готове.поле.trim(),
  ...
  const гот = adm.slice(adm.indexOf('function cardsToDraft'),
                        adm.indexOf('function cardsDownload'));
  ok(!/crmSendPic|tgSend|chatSend/.test(гот),
    'панель карток нічого не надсилає — вона тільки готує',
    'з панелі досі можна надіслати клієнту, не перечитавши');
  ok(/cardId: c\.id/.test(гот),
    'у смужці памʼятається, яка плитка яка картка — інакше позначити надіслане нічим',
    'звʼязок плитки з карткою губиться');
   ...
   документації не вгадати, тому код пробує способи сам. */
{
  ...
  const пак = adm.slice(adm.indexOf('async function chatSendPics'),
                        adm.indexOf('function picWord'));
  ok(/crmSendPic\(o, urls, pics\.map\(p => p\.blob\)\)/.test(пак),
