   ...
   документації не вгадати, тому код пробує способи сам. */
{
  ...
  /* Перші спроби — вже не здогадки. Це та сама форма, якою Sitniks САМ
     описує картинку, коли віддає її нам: mediaType/mediaUrl, а не
     type/url, як ми припускали. Читання й запис у таких API майже завжди
     однакові, тож починати треба з неї, а не з чужих звичок. */
  ok(/mediaUrl/.test(блок) && /messageType:'image'/.test(блок),
    'перебір починається з форми, якою Sitniks сам описує картинку, а не з наших здогадок',
    'у переборі немає справжніх назв полів Sitniks');
