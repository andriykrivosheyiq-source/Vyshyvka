   ...
   прорахунок сьогодні, «до цього зазвичай беруть» — через тиждень. */
{
  ...
  if(індекс < 0) console.log('  (скрипта в наборі немає — пропускаємо живу перевірку)');
    ...
    const панель = await p.evaluate(() => {
      ...
      return {
        ...
        лупа: w.querySelectorAll('[data-cd-big]').length,
        поле: (document.querySelector('.cw-inp') || {}).value || ''
      };
    });
    // Лупа відкриває свій шар у сторінці: нову вкладку на blob-адресу
    // браузер мовчки блокує, і натискання виглядало як поламане.
    const велика = await p.evaluate(async () => {
      document.querySelector('[data-cd-big]').click();
      await new Promise(r => setTimeout(r, 200));
      const d = document.getElementById('picBig');
      const on = !!(d && d.classList.contains('open') && d.querySelector('img'));
      if(d) d.click();
      return { відкрилось:on, закрилось:!(d && d.classList.contains('open')) };
    });
    /* Смужка має бути заповнена ВЖЕ ЗАРАЗ, без жодного натискання: доти
       між «натиснув заготовку» і «картка готова» стояла ще одна кнопка, і
       менеджер тиснув ➤ повз неї. Клієнту летів самий текст. */
    const готове = await p.evaluate(async () => {
      const було = { плиток: document.querySelectorAll('.cw-tray .cw-tr').length,
                     карток: (chatWin.pics || []).filter(x => x.cardId).length };
      document.querySelector('[data-cd-go]').click();
      await new Promise(r => setTimeout(r, 400));
      було.панель = !!document.querySelector('.cw-cards');
      було.поле = (document.querySelector('.cw-inp') || {}).value || '';
      return було;
    });
    ...
    /* Текст мусить бути в полі ВЖЕ ЗАРАЗ. Доти він показувався всередині
       панелі, і виглядало це як «повідомлення кудись поділось»: у полі
       порожньо, правити нічого. Андрій: «з фото видно, що повідомлення в
       блок тексту не перенесено і не можна редагувати». */
    ok(панель && /прорахунок/i.test(панель.поле),
      'текст скрипта лягає в поле одразу — його видно й можна правити',
      'у полі порожньо, текст застряг у панелі: ' + JSON.stringify(панель && панель.поле));
    ok(панель && панель.лупа === панель.плиток,
      'кожну картку можна роздивитись, не надсилаючи',
      'збільшити картку нічим: ' + JSON.stringify(панель));
    ...
    ok(панель && панель.позначених > 0 && /у полі/.test(панель.кнопка || ''),
      'потрібні картки вже позначені, і кнопка каже, скільки їх уже в полі',
      'нічого не позначено або кнопка мовчить: ' + JSON.stringify(панель));
    /* Андрій: «любі заготовки автоматом не відправляються, вони одразу
       спочатку попадають сюди в блок, де написати клієнту, потім ми
       перечитуємо, можливо, щось підкориговуємо і потім відправляємо». */
    ok(велика.відкрилось && велика.закрилось,
      'картка відкривається на весь екран просто в сторінці й так само закривається',
      'роздивитись картку не виходить: ' + JSON.stringify(велика));
    console.log('   ' + JSON.stringify(готове));
    ok(готове.плиток > 0 && готове.карток === готове.плиток,
      'картки лежать у смужці одразу, без окремої дії — ➤ надішле саме їх',
      'смужка порожня: ➤ відправить самий текст, а картка лишиться в панелі: ' + JSON.stringify(готове));
    ok(!!готове.поле.trim(),
      'а текст скрипта лишається в полі — менеджер перечитує й коригує',
      'текст скрипта нікуди не потрапив: ' + JSON.stringify(готове));
    ok(!готове.панель,
      'панель закривається — далі працюють полем, а не нею',
      'панель лишилась поверх поля вводу');
    /* Панель показує, ЩО ЛЕЖИТЬ У ПОЛІ. Коли в полі вже нічого немає, вона
       висить порожньою обіцянкою, і менеджер мимоволі чекає від неї ще
       одної дії — саме це Андрій і побачив після відправки. */
    const післяВідправки = await p.evaluate(() => {
      chatWin.cards = { kind:'kp', pick:{}, deck:{ list:[] }, err:'' };
      chatWin.pics = [];
      renderChatWin();
      const було = !!document.querySelector('.cw-cards');
      // те, що робить відправка, коли смужка спорожніла
      if(!chatPics().length) chatWin.cards = null;
      renderChatWin();
      return { було, стало: !!document.querySelector('.cw-cards') };
    });
    ok(післяВідправки.було && !післяВідправки.стало,
      'надіслали — панель зникає сама, а не висить над порожнім полем',
      'панель лишається після відправки: ' + JSON.stringify(післяВідправки));
    await p.evaluate(() => { (chatWin.pics || []).forEach(chatDropPic);
      chatWin.pics = []; chatWin.draft = ''; renderChatWin(); });
  }
  ...
  const гот = adm.slice(adm.indexOf('function cardsSync'),
                        adm.indexOf('function cardsDownload'));
  ok(!/crmSendPic|tgSend|chatSend/.test(гот),
    'панель карток нічого не надсилає — вона тільки показує, що вже в полі',
    'з панелі досі можна надіслати клієнту, не перечитавши');
  /* Хрестик ховає саму панель і нічого не чіпає: картки лишаються в полі.
     Це не те саме, що «Прибрати всі», і саме тому потрібні обидва. */
  ok(/data-cd-hide/.test(adm) && /cw-cd-x/.test(adm),
    'панель можна просто закрити хрестиком — картки при цьому лишаються в полі',
    'закрити панель, не скасувавши підготовку, нічим');
  ok(/cardId: c\.id/.test(гот) && /!st\.pick\[id\]/.test(гот),
    'галочка й смужка — одне й те саме: зняли галочку, плитка зникла',
    'галочки й смужка живуть окремо — надішлеться не те, що позначено');
  /* Instagram у Direct бере jpeg, png і gif; webp не бере. А зменшувач
     робить саме webp, коли в картинці є прозорість. */
  ok(/function picSendable/.test(adm) && /cv\.toBlob\(done, 'image\/png'\)/.test(adm),
    'webp перекладаємо в png — Instagram webp не приймає, і фото зупинялось би в CRM',
    'webp іде як є: частина фото доходить до CRM і далі не йде');
  ok(/fd\.append\('file', blob, picFileName\(blob\)\)/.test(adm),
    'у сховище файл іде з імʼям і розширенням — адреса без нього за картинку не вважається',
    'файл вантажиться без імені: адреса може лишитись без розширення');
  /* Друга галочка на кнопці КП тримається саме на цій миті: клієнт озвався
     ПІСЛЯ карток — значить розмова пішла. Ставить її та, хто знає, що
     дійшло, — сама відправка, а не панель. */
  const пік2 = adm.slice(adm.indexOf('async function chatSendPics'),
                         adm.indexOf('function picWord'));
  ok(/o\.cardsAt = now/.test(пік2) && /o\.cardsWrote = cardsWroteNow\(o\)/.test(пік2),
    'відправка лишає слід, з якого росте друга галочка на кнопці КП',
    'мить відправки не запамʼятовується — ✓✓ не зʼявиться ніколи');
  ok(/sent\[x\.cardId\] = /.test(пік2),
    'позначку «надіслано» ставить сама відправка — до неї ніхто не знає, чи дійшло',
    'картка позначається надісланою ще до того, як пішла');
  ok(/crmOutNote\(o, u\)/.test(пік2),
    'надіслана картка впізнається як наша — стане праворуч і з іменем менеджера',
    'власна картка знову стане ліворуч, як клієнтська');
  /* Свій список заготовок є в кожному робочому акаунті, і стандартний він
     не бачить зовсім: скрипти з картками, додані пізніше, не зʼявились
  ...
  ok(/contentData\.qrSeed \|\| 0\) >= QR_SEED/.test(сід),
    'доводимо один раз — свідомо прибраний скрипт назад не повернеться',
    'скрипт повертатиметься щоразу, скільки його не прибирай');
}
   ...
   документації не вгадати, тому код пробує способи сам. */
{
  ...
  /* ПРИЙНЯЛИ — ЗУПИНЯЄМОСЬ.

     Успіх доти перевірявся перечитуванням стрічки: побільшало в ній
     вкладень чи ні. Sitniks же НЕ ПОВЕРТАЄ нам наші власні вкладення —
     отже перевірка не могла спрацювати ніколи, кожен спосіб оголошувався
     невдалим, і перебір ішов далі, надсилаючи картинку ЗНОВУ. Андрій
     отримав три однакові картки поспіль, потім посилання, а текст — аж
     через дві хвилини, бо стояв у черзі за ними.

     Помилка була не в паузах, а в правилі: «прийнято» означає, що клієнту
     вже пішло, і пробувати після цього щось іще — це надсилати вдруге. */
  const pic = adm.slice(adm.indexOf('async function crmSendPic'),
                        adm.indexOf('async function crmSend(o, text)'));
  ok(!/crmPicCount/.test(pic) && /crmPicLog\.push\(w\.id \+ ' → ПРИЙНЯВ'\)/.test(pic),
    'спосіб, який відповів «прийнято», — останній: далі нічого не пробуємо',
    'після «прийнято» перебір іде далі — клієнт отримає ту саму картинку вдруге');
  ok(/if\(!ok\) continue;/.test(pic) && pic.indexOf('if(!ok) continue;') < pic.indexOf('return w.id'),
    'наступний спосіб пробуємо лише після справжньої відмови',
    'наступний спосіб пробується й після вдалої відправки');
  ...
  /* Пачка складається ДО відправки: спершу всі картки у сховище, потім
     ...
     окремі повідомлення. */
  const пак = adm.slice(adm.indexOf('async function chatSendPics'),
                        adm.indexOf('function picWord'));
  /* Андрій: «повинна бути тільки картинка». Синє посилання в Direct майже
     ніхто не відкриває — тобто замість картки клієнт отримував порожнє
     місце. Тож не пройшла картинка — не надсилаємо нічого, а повертаємо
     все менеджеру: половина відправки гірша за жодної. */
  ok(!/crmSend\(o, u\)/.test(пак) && /back\(\);\s*\n\s*chatWin\.draft = text/.test(пак),
    'не пройшла картинка — нічого не надсилаємо: ні посилання, ні самого тексту',
    'замість картинки клієнту летить синє посилання, якого він не відкриє');
  ok(/crmSendPic\(o, urls, blobs\)/.test(пак),
    'усі зібрані картинки їдуть у Sitniks одним повідомленням — пачкою, як пише людина',
    'картинки досі йдуть по одній: клієнту прилітає черга окремих сповіщень');
   ...
   клієнтське. Прикмета — номер повідомлення з відповіді на нашу відправку. */
{
  ...
  /* Картку між відправкою й перечитуванням ЗАМІНЮЄ знімок бази: він
     приходить за секунду й привозить ще ту версію, у якій нашого запису
     немає. Доки збереження долетить назад, власне повідомлення встигало
     стати ліворуч і без підпису — саме це й було видно в робочій адмінці. */
  const свіжа = await p.evaluate(() => {
    const свіжий = { id:'1', name:'Оксана' };     // та сама картка, новий обʼєкт
    const m = (crmNormMsgs([{ id:'m-77', attachments:['https://lookaside.fbsbx.com/x'],
      createdAt:'2026-09-22T10:00:00.000Z' }], свіжий) || [])[0] || {};
    return { наше:!!m.mine, підпис:m.by || '' };
  });
  console.log('   ' + JSON.stringify(свіжа));
  ok(свіжа.наше && !!свіжа.підпис,
    'прикмета переживає підміну картки знімком бази — бік і підпис не злітають',
    'після оновлення картки власне повідомлення знову ліворуч: ' + JSON.stringify(свіжа));
}
