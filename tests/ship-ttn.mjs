...
console.log('');
console.log('═══ БЕЗ ПОГОДЖЕННЯ ФОТО НАКЛАДНОЇ НЕМАЄ ═══');
/* Останні ворота ланцюга й найдорожчі: після відправки виправляти нема
   чого. Умова одна, зате тверда — акаунт-менеджер подивився фото готової
   партії й сказав «так». Не «виробництво вважає, що все добре»:
   виробництво завжди вважає, що все добре, воно щойно це пошило. */
const gate = await p2.evaluate(async () => {
  const o = orders[0];
  o.ttn = ''; o.ttnAt = '';
...
ok(!gate.рано && gate.післяРано === '',
  ...
  'накладна створилась до погодження: «' + gate.післяРано + '»');
ok(!gate.брак && /брак/.test(gate.чомуБрак),
  'партія в браку теж не їде — спершу переробка',
  'бракована партія пройшла ворота');
ok(gate.можна && gate.номер === '20450000000002',
  'погодив — накладна записується, як і раніше',
  'після погодження накладна не записалась: «' + gate.номер + '»');
