...
/* Пункти списку — іконка + назва + шеврон, як у переліку розділів
   застосунку. Без іконки шість однакових рядків читались суцільною стіною
   тексту; підпису під назвою немає навмисно — назви достатньо. */
const fr = await p.frames()[1].evaluate(() => {
  const r = document.querySelector('.fold .fr');
  return { іконка: !!(r && r.querySelector('.fr-ic svg')),
           підпис: !!(r && r.querySelector('.fr-t i')),
           кегль: r ? getComputedStyle(r.querySelector('.fr-t')).fontSize : '' };
});
console.log('  ' + JSON.stringify(fr));
ok(fr.іконка && !fr.підпис,
  'у пункті є іконка й немає зайвого підпису під назвою',
