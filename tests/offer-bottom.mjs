/* Низ пропозиції: заклик одразу за сумою, під ним — рівно те, що знімає
   ...
   кошторис → згортки → менеджер → коротка кнопка в кінці.
...
ok(d.гарантій === 4,
  ...
  'галочок: ' + d.гарантій);
/* Рядка «Відгуки» тут немає навмисно. Пропозиція іменна: людина вже
   спілкується з менеджером, і відсилати її на загальну сторінку відгуків
   посеред власного кошторису — це вести її ГЕТЬ зі сторінки за крок до
   кнопки. Разом із рядком пішла й смужка фото під ним. */
ok(!d.рядки.some(x => /Відгук/i.test(x.назва)) && d.смужкаФото === 0,
  'відгуків у списку немає — зі сторінки пропозиції нікуди не відводимо',
  'відгуки повернулись: ' + JSON.stringify(d.рядки.map(x => x.назва)) +
    ', фото ' + d.смужкаФото);
...
/* Окремого блоку «Готові оформити замовлення?» більше немає. Він повторював
   кнопку з кошторису — ту саму дію, тільки на екран нижче й під новим
   заголовком. Друга кнопка з тією самою дією не додає рішучості: людина
   бачить, що її ще про щось просять. */
ok(d.закликів === 0,
  'окремого фінального заклику немає — дія живе в кошторисі, поруч із сумою',
  'заклик повернувся: ' + d.закликів);
ok(d.порядок.кошторис < d.порядок.список && d.порядок.список < d.порядок.менеджер,
  'порядок: кошторис → згортки → менеджер',
  'порядок збився: ' + JSON.stringify(d.порядок));
/* Рівно два рядки. Кожен зайвий тут коштує дорого: він стоїть між сумою й
   кнопкою. «Умови замовлення» переказували шапку, проєкти й питання — це
   матеріал сайту, а не цієї пропозиції. */
const names = d.рядки.map(x => x.назва);
ok(names.length === 2 && names[0] === 'Етапи виконання замовлення' &&
   names[1] === 'Гарантії вашого замовлення',
  'у списку рівно два пункти: як це буде і що буде, якщо вийде не так',
...
ok(e.рядки.length === 2,
  'обидва пункти на місці — вони не залежать від того, що заповнив менеджер',
...
/* Пункт списку — назва й шеврон, більше нічого. Іконки перед назвами
   пробували: шість різних значків перетягували на себе увагу й читались як
   меню застосунку, хоч це просто зміст під кошторисом. Підпису під назвою
   немає з тієї самої причини. */
const fr = await p.frames()[1].evaluate(() => {
  const r = document.querySelector('.fold .fr');
  return { іконка: !!(r && r.querySelector('.fr-ic')),
           підпис: !!(r && r.querySelector('.fr-t i')),
           кегль: r ? getComputedStyle(r.querySelector('.fr-t')).fontSize : '' };
});
console.log('  ' + JSON.stringify(fr));
/* Кегль на крок більший за текст сторінки: рядків лишилось два, і кожен —
   відповідь на страх клієнта, а не службова примітка внизу. */
ok(!fr.іконка && !fr.підпис && fr.кегль === '17px',
  'у пункті сама назва — без іконки й підпису, кеглем ' + fr.кегль,
  'пункт не той: ' + JSON.stringify(fr));
