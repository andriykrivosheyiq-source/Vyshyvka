...
(function(){
  ...
  function buildCards(offer, cfg){
    ...
    var shotsOf = function(it){
      ...
      /* ОБОВʼЯЗКОВІ ДВА КАДРИ — ПЕРЕД І ЗАД. Далі додаються сторони, на
         яких справді є нанесення: бік, рукав, плече. Аркуш під кожен такий
         кадр розширюється вбік.

         Був період, коли спина вважалась необовʼязковою: є фото моделі —
         і досить. Андрій: «головне, щоб було завжди дві картинки, за базу
         перед і зад». І це правда про сам виріб: людина, яка дивиться на
         футболку спереду, наступним питанням питає, що ззаду, — байдуже,
         нанесено там щось чи ні. Порожня спина відповідає на це питання
         так само добре, як спина з логотипом.

         Фото моделі цього не скасовує: воно показує, як виріб сидить, а не
         який він, і за замовчуванням лишається знятим. */
      add(byId.front);
      add(byId.back);
      var lost = [];
    ...
    var card = function(id, kind, it){
      ...
      /* ДВА КАДРИ — НИЖНЯ МЕЖА КАРТКИ. Зазвичай це перед і зад; але в
         частини виробів заднього ракурсу просто немає — його не зняли, і
         взяти нізвідки. Тоді добираємо будь-чим, що в позиції є: інший
         ракурс, мокап, фото моделі. Картка з однією картинкою виглядає
         недоробленою незалежно від того, чому вона така.

         Явно зняте менеджером не чіпаємо: він прибрав цей кадр свідомо, і
         повернути його «бо треба два» означало б сперечатись із ним. */
      if(keep.length < 2){
        var явно = (own && own.off) || {};
        /* Черга добору: спершу ще один ракурс самого виробу, потім мокап,
           і лише наостанок фото моделі. Питання, на яке відповідає другий
           кадр, — «а який він з іншого боку», і фото моделі відповідає на
           нього гірше за будь-який ракурс. */
        var черга = shots.filter(function(sh){ return !sh.model && sh.side; })
              .concat(shots.filter(function(sh){ return !sh.model && !sh.side; }))
              .concat(shots.filter(function(sh){ return sh.model; }));
        черга.forEach(function(sh){
          if(keep.length >= 2) return;
          if(keep.indexOf(sh) >= 0) return;
          if(явно[sh.key]) return;
          keep.push(sh);
        });
      }
      var allShots = shots;
  ...
  function trimOf(img){
    ...
    try{
      ...
      /* ФОН — ЦЕ ТЕ, ЩО В КУТАХ, А НЕ ОБОВʼЯЗКОВО БІЛЕ.

         Тут довго стояло одне правило: фоном вважалось прозоре й «майже
         біле». Але мокапи знімають на світло-сірому, беручому, теплому
         папері — і жоден із цих фонів під «майже біле» не підпадає. Тоді
         межі виробу не знаходились узагалі, за виріб бралася вся картинка з
         її полями, і далі ламалось усе, що від цих меж залежить: масштаб
         ряду підбирався під кадр, а не під виріб, і виріб виходив то
         крихітним у великій плитці, то підрізаним знизу. Найгірше, що
         виправлення розкладки цього не лікували — вони працювали з межами,
         яких не було.
  ...
  function drawShot(x, sr, k, X, Y, bw, bh, ky){
    ...
    var ih = im.height * k, gh = tr.h * k;
    var y = Y + bh / 2 - cy * k;
    /* ВИРІБ ЦІЛИЙ — ЦЕ УМОВА, А НЕ ПОБАЖАННЯ.

       Тут стояло правило «верх кадру недоторканний»: кадр вищий за плитку —
       притискаємо його до верху, зайве лишаємо знизу. Воно правильне для
       фото МОДЕЛІ, де не можна різати голову. На пласкому мокапі воно
       робило протилежне: кадр вищий за плитку майже завжди, тож правило
       щоразу скасовувало центрування по виробу, і куди поїде сам виріб,
       вирішували поля мокапа. Поля в них несиметричні — верхнє більше за
       нижнє, — і виріб з'їжджав донизу, де плитка його й відрізала.
       Клієнт отримував картку з обрізаним светром.

       Масштаб ряду й так підібраний так, щоб виріб вліз у плитку цілком
       (rowZoom бере найменший із двох обмежень). Лишалось не зіпсувати це
       позиціюванням: ставимо виріб по центру плитки й тримаємо в її межах.
       Повзунок «Рівень кадру» рухає його у вільному місці — але за край
       не виводить ніколи. */
    var рівень = Math.max(0, Math.min(1, +ky || 0));
    /* ПІВПІКСЕЛЯ ДОПУСКУ — І ЦЕ НЕ ДРІБНИЦЯ.

       Масштаб ряду підбирають так, щоб найбільший виріб рівно вліз у плитку:
       найчастіше `gh` виходить не «менше за bh», а РІВНО bh — з точністю до
       частки пікселя. Без допуску звичайне 890.0000001 > 890 відправляло
       найтиповіший випадок у запасну гілку, де кадр притискається до верху
       плитки. А кадр — це не виріб: у мокапі над виробом порожнє поле. Виріб
       з'їжджав вниз рівно на висоту того поля, і плитка відрізала йому низ —
       при тому, що за розміром він уміщався ідеально. */
    if(gh <= bh + 0.5){
      var slack = Math.max(0, bh - gh);
      if(ky) y += (рівень - 0.5) * slack;
      var top = y + tr.y * k;
      if(top < Y) y += Y - top;
      else if(top + gh > Y + bh) y -= top + gh - (Y + bh);
    } else {
      /* Виріб не вміщається навіть за найменшого масштабу — вибір між двома
         втратами. Ділимо зайве за повзунком, але порожнього поля в плитці не
         лишаємо: виріб має перекрити її цілком. */
      var over = gh - bh;
      y -= (рівень - 0.5) * over;
      var t2 = y + tr.y * k;
      if(t2 > Y) y -= t2 - Y;
  ...
  function shotsGrid(x, imgs, card, t, show, o, X, Y, w, h){
    ...
    /* ЧЕТВЕРТА КЛІТИНКА ПРИ ТРЬОХ КАДРАХ ЛИШАЄТЬСЯ ПОРОЖНЬОЮ — і це
       ...
       нас і так скрізь. */
    /* Масштаб рахуємо по ВСІХ кадрах, які справді намалюємо, а не по двох
       перших. Доти тут стояло slice(0, 2): якщо виріб у третьому чи
       четвертому кадрі займав більшу частку свого кадру, ніж у перших двох,
       у плитку він не вміщався — і його різало. Масштаб спільний на сітку,
       отже й обмеження має бути спільним. */
    var Z = rowZoom(srcs.slice(0, 4), cw, ch);
  ...
  /* НАЇЗД БІЛЬШЕ НЕ ПОТРІБЕН — І САМЕ ВІН РІЗАВ ВИРІБ.

     Тут стояло 1.2: «мокапи приходять із власним запасом навколо виробу, і
     на смузі він читається як порожнеча — трохи наїжджаємо». Міркування
     було правильне для того часу, коли межі виробу не знаходились узагалі
     (фон мокапа світло-сірий, а фоном вважалось лише прозоре й майже біле).
     Кадр тоді вписувався в плитку цілком, разом із полями, і виріб справді
     виглядав дрібним — наїзд це компенсував.

     Щойно межі запрацювали, масштаб і так підбирається ПО ВИРОБУ: він
     заповнює плитку без полів. А наїзд поверх цього почав різати — саме те,
     що видно на картці порівняння: у худі відрізано низ і рукав.

     Лишаємо один до одного: виріб має бути цілим. Порожнечі навколо нього
     більше немає й без наїзду. */
  var CMP_ZOOM = 1;
  ...
  async function drawCard(card, offer, cfg){
    ...
    /* ═══ АРКУШ МАЛЮЄТЬСЯ ВДВІЧІ ГУСТІШЕ, НІЖ РОЗКЛАДАЄТЬСЯ ═══
       Уся розкладка тут рахується в логічних точках: смуги, відступи,
       кеглі. Раніше в стільки ж пікселів вона й малювалась — і знімок
       виробу на 1600 px стискався в плитку на шістсот. Разом із ним
       стискався логотип: на екрані телефона від нього лишалась пляма, і
       саме її клієнт отримував у Direct.

       Тому полотно беремо вдвічі більшим, а систему координат масштабуємо
       назад. Жодне число в розкладці не міняється — міняється тільки
       щільність пікселів: текст перестає замилюватись, а знімок віддає
       вдвічі більше своєї справжньої роздільності. CSS-розмір лишаємо
       логічним, щоб картка на екрані виглядала так само, як виглядала. */
    var S = 2;
    var cv = document.createElement('canvas');
    cv.width = W * S; cv.height = H2 * S;
    cv.style.width = W + 'px'; cv.style.height = H2 + 'px';
    var x = cv.getContext('2d');
    x.scale(S, S);
    /* Знімки приходять більшими за плитку — отже кожен ще й зменшується.
       Без цього рядка браузер робить це найдешевшим способом, і дрібні
       деталі — а логотип саме такий — просто зникають. */
    ...
    /* Скільки пікселів на логічну точку. Потрібно всім, хто міряє аркуш у
       його власних одиницях, — і насамперед перевіркам розкладки. */
    cv.lqScale = S;
    cv.lqNotes = notes;
    return cv;
  }
  ...
  window.LQCards = {
    ...
    /* Чи стоїть цей кадр на картці. Віддаємо назовні навмисно: правило
       живе тут, а галочки в робочому місці малює кадр пропозиції — і своя
       копія цього правила там уже одного разу розійшлась. Розходження це
       найгіршого штибу: менеджер бачить кадр позначеним, а на картці його
       немає, ставить галочку ще раз і вирішує, що робоче місце не
       зберігає. */
    shotOn: shotOn
  };
})();
