...
(function(){
  ...
  function drawShot(x, sr, k, X, Y, bw, bh, ky){
    ...
    if(gh <= bh){
      var slack = bh - gh;
      if(ky) y += (Math.max(0, Math.min(1, +ky)) - 0.5) * slack;
      var top = y + tr.y * k;
      if(top < Y) y += Y - top;
      else if(top + gh > Y + bh) y -= top + gh - (Y + bh);
    } else if(ih > bh){
      /* Виріб не вміщається навіть за найменшого масштабу — тоді зайве йде
         знизу, як і раніше: це вже вибір між двома втратами. */
      y = Y - (ih - bh) * (+ky || 0);
    }
